        ...
        public void Execute(IServiceCollection services)
        {
            foreach (var typeItem in this.typeData.Where(x => x.Value.Pattern == TypePattern.MarshalledStruct))
            {
                ...
                foreach (var member in type.Members)
                {
                    ...
                    this.patternRules.ApplyFirst(type.Members, member, new MemberPatternContext(null, false, type.Extension, x => Default(this.nameLookup.Lookup(new TypeReference { VkName = x }, false)), typeItem.Key), patternInfo);
