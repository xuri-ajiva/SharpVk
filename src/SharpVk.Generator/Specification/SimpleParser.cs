...
namespace SharpVk.Generator.Specification
{
    public static class SimpleParser
    {
        ...
        private static readonly Parser<LenExpressionToken> lenTokenParser = Parse.Letter.AtLeastOnce().Text().Select(x => new LenExpressionToken
        {
            Value = x
        });
        ...
        private static readonly Parser<LenExpressionLiteral> integerLiteral = from sign in Sign
            ...
            select new LenExpressionLiteral
            {
                Value = sign + digits
            };
        ...
        private static readonly Parser<LenExpressionToken> latexToken = Parse.LetterOrDigit.AtLeastOnce()
            ...
            .Text()
            .Token()
            .Select(x => new LenExpressionToken
            {
                Value = x
            });
        ...
        private static readonly Parser<LenExpression> latexUnaryOperatorExpression = latexSubexpressionRef.Contained(Parse.String("\\lceil{"), Parse.String("}\\rceil")).Token().Select(x => new LenExpressionOperator
        {
            LeftOperand = x,
            Operator = LenOperatorType.Ceiling
        });
        ...
        private static readonly Parser<LenExpression> latexMultiplicativeExpression = Parse.ChainOperator(Parse.String("\\over").Token(),
            ...
            (op, left, right) => new LenExpressionOperator
            {
                LeftOperand = left,
                RightOperand = right,
                Operator = LenOperatorType.Divide
            });
        ...
        private static readonly Parser<MemberLen> lenPartParser = Parse.String("null-terminated").Select(x => new MemberLen
            {
                Type = LenType.NullTerminated
            })
            .Or(lenExpressionParser.Select(x => new MemberLen
            {
                Value = x,
        ...
        public static MemberLen[] ParsedLenField(string field)
        {
            if (field.Contains("->d"))
            {
                return ParsedLenField( /*field.Split("->")[1]*/ "pDescriptorSets");
            }
            if (field.Contains("->c"))
            {
                return ParsedLenField( /*field.Split("->")[1]*/ "pCommandBuffers");
            }
            if (field.Contains("*"))
            {
                var parts = field.Split("*");
                var multy = int.Parse(parts[0]);
                int sizN = (parts[1])switch
                {
                    "VK_UUID_SIZE" => 128,
                ...
                return new[]
                {
                    new MemberLen
                    {
                        Value = new LenExpressionLiteral()
                        {
                            Value = (multy * sizN).ToString()
                        },
                        Type = LenType.Expression
                    }
                };
